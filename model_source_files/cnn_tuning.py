...
import csv
import os
from cnn_model import build_cnn_ffnn_model_with_resnet  # Adjust based on actual module import path
from data_loader import create_data_loaders
import torch.optim as optim

# Define results file path
results_file = "/home/pmanmat/scratch/tldo/data/hp_tuning_out_cnn_ffnn.csv"
# results_file = "hp_tuning_out_cnn_ffnn.csv"
...
if not os.path.exists(results_file):
    with open(results_file, mode="w", newline="") as file:
        ...
        writer.writerow(["trial", "cnn_filters", "embedding_dim", "ffnn_hidden_dim", "dropout", "learning_rate", "val_loss"])
        print("CSV Created")
...
def objective(trial):
    ...
    ffnn_hidden_dim = trial.suggest_int("ffnn_hidden_dim", 32, 256, step=32)
    dropout = trial.suggest_float("dropout", 0.1, 0.5)
    learning_rate = trial.suggest_loguniform("learning_rate", 1e-5, 1e-2)

    print(f"Starting trial {trial.number} with parameters: "
          f"cnn_filters={cnn_filters}, embedding_dim={embedding_dim}, "
    ...
    # # Load data
    # train_loader, val_loader, _ = create_data_loaders(
    #     train_csv_file="../output_files/London_ON_09_23_results.csv",  # Adjusted path
    #     train_weather_file="../weather_files/CAN_ON_London_2009_2023.epw",  # Adjusted path
    #     test_csv_file="../output_files/London_ON_04_18_results.csv",  # Adjusted path
    ...
    # Initialize model
    model = build_cnn_ffnn_model_with_resnet(
    ...
    with open(results_file, mode="a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([trial.number, cnn_filters, embedding_dim, ffnn_hidden_dim, dropout, learning_rate, avg_val_loss])
